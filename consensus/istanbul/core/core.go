...
func (c *core) startNewRound(newView *istanbul.View, roundChange bool) {
	...
	c.current = newSnapshot(newView, c.valSet)
	// Calculate new proposer
	c.valSet.CalcProposer(c.proposerSeed())
...
func (c *core) catchUpRound(view *istanbul.View) {
	...
	c.current = newSnapshot(view, c.valSet)
	c.newRoundChangeTimer()
...
func (c *core) proposerSeed() uint64 {
	...
	offset := 0
	if idx, val := c.valSet.GetByAddress(c.lastProposer); val != nil {
		offset = idx
	}
